...
float sim_time, time_next_event[6];
int next_event_type;
...
//update_time_avg_stats에 나오는 변수 선언
float area_num_in_server_q,  area_server_status;
...
int main(void) {
	...
	//파일 읽고 쓰기 
	infile = fopen("system4_input.in", "r");
	outfile = fopen("system4_output30.out", "w");

	//event 1 = 고객 도착 , 2 = 키오스크1의 고객 출발, 3 = 키오스크2의 고객 출발, 4 = 서버의 고객 출발, 5 = fake event 
	num_events = 5;
...
void arrive(void) {
	...
	//일반고객의 경우
	if (cust_payco == 0) {
		// 서버큐의 길이가 각각의 키오스크큐에 있는 사람보다 3명 이상 더 많으면 키오스크 이용
		if (num_in_kiosk1_q > num_in_kiosk2_q) {
			if (num_in_kiosk2_q + 3 < num_in_server_q)
			...
			if (num_in_kiosk1_q + 3 < num_in_server_q)
				kiosk1_arrive(cust_payco);
			else
				server_arrive();
		}
...
void server_arrive() {
	...
	if (server_status == BUSY) {
		...
		//server_depart 시각 설정 
		time_next_event[4] = sim_time + normal_service(server_k, server_alpha, server_beta);
	}
}
void server_depart() {
	int i;
...
void report() {
	...
	fprintf(outfile, "키오스크1 QUEUE의 평균 delay 시간 \t\t\t %11.3f seconds\n", total_of_kiosk1_delays / num_custs_kiosk1_delayed);
	...
	fprintf(outfile, "전체 QUEUE의 평균 delay 시간 \t\t\t\t\t %11.3f seconds\n", (total_of_kiosk1_delays + total_of_kiosk2_delays + total_of_server_delays) / (num_custs_kiosk1_delayed + num_custs_kiosk2_delayed + num_custs_server_delayed));
	...
	fprintf(outfile, "전체 QUEUE의 평균 사람 수 \t\t\t\t\t\t %10.3f명\n", (area_num_in_kiosk1_q + area_num_in_kiosk2_q + area_num_in_server_q) / (sim_time));
	...
	fprintf(outfile, "전체 system에서 delay를 마친 고객 수 \t\t\t\t %d 명\n", num_custs_kiosk1_delayed + num_custs_kiosk2_delayed + num_custs_server_delayed);
	...
	fprintf(outfile, "키오스크1 에서 delay 시간이 3분(180초) 이상인 사람 수의 비율 \t\t\t %f %%\n", (float)(kiosk1_num_delay_over_3) / (float)(num_custs_kiosk1_delayed));
	fprintf(outfile, "키오스크1 에서 delay 시간이 4분(240초) 이상인 사람 수의 비율 \t\t\t %f %%\n", (float)(kiosk1_num_delay_over_4) / (float)(num_custs_kiosk1_delayed));
	fprintf(outfile, "키오스크2 에서 delay 시간이 3분(180초) 이상인 사람 수의 비율 \t\t %f %%\n", (float)(kiosk2_num_delay_over_3) / (float)(num_custs_kiosk2_delayed));
	fprintf(outfile, "키오스크2 에서 delay 시간이 4분(240초) 이상인 사람 수의 비율 \t\t %f %%\n", (float)(kiosk2_num_delay_over_4) / (float)(num_custs_kiosk2_delayed));
	fprintf(outfile, "전체 시스템에서 3분 이상 기다린 사람의 비율 \t\t\t\t %f %% \n", (float)(kiosk1_num_delay_over_3 + kiosk2_num_delay_over_3 + server_num_delay_over_3) / (float)(num_custs_kiosk1_delayed + num_custs_kiosk2_delayed + num_custs_server_delayed));
	fprintf(outfile, "전체 시스템에서 4분 이상 기다린 사람의 비율 \t\t\t\t %f %% \n", (float)(kiosk1_num_delay_over_4 + kiosk2_num_delay_over_4 + server_num_delay_over_4) / (float)(num_custs_kiosk1_delayed + num_custs_kiosk2_delayed + num_custs_server_delayed));
